__all__='CanvasAdapter','TextObject'
...
class CanvasAdapter:
    def __init__(self,baseCanvas,trans={},**kw):
        self.__trans = trans
        self.__transKeys = list(trans.keys())
    ...
    def textOut(self,text):
        ...
        if not xws:
            canv.drawString(self.__textX,self.__textY,text)
    ...
    def __getattr__(self,name):
        if not (name.startswith('_CanvasAdapter__') or name.startswith('_%s__'%self.__class__.__name__)):
            try:
                if name in self.__transKeys:
                    t = self.__trans[name]
                    if isinstance(t, collections.Callable): return t(self.__canv,name)
                    else:
    ...
    def __setattr__(self,name,value):
        ...
        elif name in self.__transKeys:
            t = self.__trans[name]
            if isinstance(t, collections.Callable): t(self.__canv,name,value)
            else:
...
class PMCanvasAdapter(CanvasAdapter):
    ...
    def __init__(self,pmCanv):
        ...
        ctm = pmCanv.ctm
        CanvasAdapter.__init__(self,pmCanv,trans=D)
        self.ctm = ctm
...
class FlowableCanvasAdapter(Flowable):
    '''Adapts a Flowable to it's drawing canvas'''
    def __init__(self,F):
        self.__F = F
    ...
    def wrap(self,aW,aH):
        try:
            ...
            self.__w, self.__h = self.__F.wrap(aW,aH)
            return self.__w, self.__h

    def split(self,aW,aH):
        return self.__F.split(aW,aH)
